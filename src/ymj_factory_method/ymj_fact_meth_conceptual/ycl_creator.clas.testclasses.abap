CLASS ltcl_creator DEFINITION FINAL FOR TESTING
  ...
  " creator sub-classes
  PRIVATE SECTION.
    DATA cut TYPE REF TO ycl_creator.
...
CLASS ltcl_creator IMPLEMENTATION.
  ...
  METHOD should_create_product_a.
    ...
    cut ?= NEW ycl_concrete_creator_a( ).
    DATA(product_a) = cut->create_product( ).
  ...
  METHOD should_create_product_b.
    ...
    cut ?= NEW ycl_concrete_creator_b( ).
    DATA(product_b) = cut->create_product( ).
